...
class InputData:
    def __init__(self, input_file_name, min_count):
        self.input_file_name = input_file_name  #数据文件名
        self.index = 0
        self.input_file = open(self.input_file_name, "r", encoding="utf-8")
        self.min_count = min_count  #如果词语的出现频率小于min_count就忽略这个词语
        self.wordid_frequency_dict = dict()  #key为id，value为词语的出现频率
        self.word_count = 0  #词的总数
        self.word_count_sum = 0
        self.sentence_count = 0
        self.id2word_dict = dict()  #key为id，value为word
        self.word2id_dict = dict()  #key为word，value为id
    ...
    def _init_sample_table(self): #用一个数组存储词语的id，出现频率越高在数组中的元素越多（但不是线性增长的）
    ...
    def get_wordId_list(self): #用id来代替word表示句子
    ...
    def get_batch_pairs(self, batch_size, window_size): #生成batch_size个词对，词对中的词的距离符合窗口的大小
...
if __name__=="__main__":
    test_data = InputData('../data/text.txt', 1)
