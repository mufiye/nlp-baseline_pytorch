...
class CBOWModel(nn.Module):
    ...
    def _init_embedding(self):
        ...
        self.u_embeddings.weight.data.uniform_(-int_range, int_range) #均匀分布的初始化方式
    ...
    def forward(self, pos_u, pos_w, neg_w):
        ...
        pos_u_emb = torch.FloatTensor(pos_u_emb)  # 转为tensor 大小 [ mini_batch_size * emb_size ]
        pos_w_emb = self.w_embeddings(torch.LongTensor(pos_w))  # 转换后大小 [ mini_batch_size * emb_size ]
        neg_w_emb = self.w_embeddings(torch.LongTensor(neg_w))  # 转换后大小 [ mini_batch_size*negative_sampling_number  * emb_size ]
